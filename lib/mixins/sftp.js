...
Sftp.prototype.fastGet = function (options) {
    ...
    if (_.isNil(remotePath)) {
        throw new Error("Missing parameter: remotePath");
    }
    remotePath = _path.resolve(ssh2Client._remoteDirectory, remotePath);
    var localPath = _.get(options, 'localPath');
    if (_.isNil(localPath)) {
        throw new Error("Missing parameter: localPath");
    }
...
Sftp.prototype.readdir = function (options) {
    ...
    var details = _.get(options, 'details');
                ...
                try {
                    sftp.readdir(path, function (err, files) {
                        ...
                        if (details) {
                            ...
                            return resolve(_.map(files, 'filename'));
                        }
                    })
                } catch (err) {
                    return reject(err);
...
Sftp.prototype.rename = function (options) {
    ...
    if (_.isNil(srcPath)) {
        throw new Error("Missing parameter: srcPath");
    }
